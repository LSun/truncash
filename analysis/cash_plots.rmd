...
# Noise <- cut(sd.z, breaks = c(0, z.under, z.over, Inf), labels = c("Deflated Noise", "In-between", "Inflated Noise"))
  ...
  joint <- plot_grid(FDR.calib.plot,
            pi0.plot + rremove("x.text"),
            FDR.plot + rremove("x.text"),
            power.plot,
            align = "v", ncol = 1, nrow = 4,
            rel_heights = c(1.5, 1, 1, 1.2)
  )
  annotate_figure(joint,
    top = bquote(pi[0] == .(pi0.vec[j]))
  )
  ggsave(paste0("../output/fig/pi0_", pi0.vec[j], ".eps"), height = 10, width = 8)
}
```
