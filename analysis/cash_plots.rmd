...
```{r sim_z_2, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE, eval = FALSE}
for (sigma.beta in 5) {
for (w.lambda in 10) {
# cat("sigma.beta =", sigma.beta, "; GD =", L, "; Constraints on w:", w.lambda != 0)
qvalue.pi0.list <- qvalue.list <- list()
...
for (i in 1 : nsim) {
  ...
  ## different methods
  fit.ash = ashr::ash(betahat, sebetahat, mixcompdist = "normal", method = "fdr")
  L <- ifelse(mean(abs(betahat / sebetahat)) < sqrt(2 / pi), 4, 10)
...
for (j in seq(length(pi0.vec))) {
  ...
  FDR.plot <- ggplot(data = melt(cbind(Noise, as.data.frame(FDP.array[, which(round(q.vec, 2) == q), , j])), id.vars = "Noise"),
         aes(x = variable, y = value, col = variable)) +
  geom_boxplot() +
  ...
  power.plot <- ggplot(data = melt(cbind(Noise, as.data.frame(power.array[, which(round(q.vec, 2) == q), , j])), id.vars = "Noise"),
        aes(x = variable, y = value, col = variable)) +
  geom_boxplot() +
