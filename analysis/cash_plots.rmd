...
```{r g}
plotx <- seq(-10, 10, by = 0.01)
ploty.0 <- dnorm(plotx)
ploty.1 <- dnorm(plotx, 0, 2)
ploty.2 <- 0.5 * dnorm(plotx, -2, 1) + 0.5 * dnorm(plotx, 2, 1)
ploty.3 <- 0.4 * dnorm(plotx, 0, 0.5) +
  0.2 * dnorm(plotx) + 0.2 * dnorm(plotx, 0, 2) +
  0.2 * dnorm(plotx, 0, 3)
ploty.4 <- sapply(plotx, function(x) {mean(dnorm(x, seq(-3, 3, by = 0.5), 0.5))})
ploty.5 <- 0.6 * dnorm(plotx) + 0.4 * dnorm(plotx, 0, 3)
ploty.6 <- dnorm(plotx, 0, 5)
ploty.7 <- 0.25 * dnorm(plotx, -2, 2) + 0.25 * dnorm(plotx, -1, 1.5) + 1 / 3 * dnorm(plotx) + 1 / 6 * dnorm(plotx, 1, 1)
ploty <- cbind(ploty.0, ploty.1, ploty.2, ploty.3, ploty.4, ploty.5, ploty.6, ploty.7)
plot(plotx, plotx, type = "n", xlim = c(-6, 6), ylim = range(ploty), xlab = expression(theta), ylab = expression(g(theta)))
for (j in seq(ncol(ploty))) {
  lines(plotx, ploty[, j], col = j)
}
```
...
## Spiky: $g_3 = 0.4 N\left(0, 0.5^2\right) + 0.2 N\left(0, 1^2\right) + 0.2 N\left(0, 2^2\right) + 0.2 N\left(0, 3^2\right)$
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round((1 - pi0) * 0.4 * ngene), 0, 0.5 * se),
    rnorm(round((1 - pi0) * 0.2 * ngene), 0, 1 * se),
    rnorm(round((1 - pi0) * 0.2 * ngene), 0, 2 * se),
    rnorm(round((1 - pi0) * 0.2 * ngene), 0, 3 * se)
    ))
  betahat <- beta + sebetahat * z
...
## Near normal: $g_5 = 0.6 N\left(0, 1^2\right) + 0.4 N\left(0, 3^2\right)$
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round((1 - pi0) * 0.6 * ngene), 0, 1 * se),
    rnorm(round((1 - pi0) * 0.4 * ngene), 0, 3 * se)
    ))
  betahat <- beta + sebetahat * z
...
## Big normal: $g_6 = N\left(0, 5^2\right)$
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round((1 - pi0) * ngene), 0, 5 * se)
    ))
  betahat <- beta + sebetahat * z
...
## Skew: $g_7 = 1/4 N\left(-2, 2^2\right) + 1/4 N\left(-1, 1.5^2\right) + 1/3 N\left(0, 1^2\right) + 1 / 6 N\left(1, 1^2\right)$
...
for (i in 1 : nsim) {
  ...
  se <- sqrt(mean(sebetahat^2))
  beta <- c(
    rnorm(round((1 - pi0) * ngene / 4), -2, 2 * se),
    rnorm(round((1 - pi0) * ngene / 4), -1, 1.5 * se),
    rnorm(round((1 - pi0) * ngene / 3), 0, 1 * se),
    rnorm(round((1 - pi0) * ngene / 6), 1, 1 * se)
  )
  beta <- sample(c(
    rep(0, ngene - length(beta)),
    beta
    ))
  betahat <- beta + sebetahat * z
...
## Small normal: $g_8 = N\left(0, 1^2\right)$

```{r gtex_z_8, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
set.seed(777)
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round((1 - pi0) * ngene), 0, 1 * se)
    ))
  betahat <- beta + sebetahat * z
...
## Small normal: $g_9 = N\left(0, 1.5^2\right)$

```{r gtex_z_9, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
set.seed(777)
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round((1 - pi0) * ngene), 0, 1.5 * se)
    ))
  betahat <- beta + sebetahat * z
