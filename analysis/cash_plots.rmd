...
Rho <- cov2cor(Sigma)
par(mar = c(5, 5, 1, 1))
hist(Rho[lower.tri(Rho)], breaks = 100, xlab = expression(rho[ij]), xlim = c(-1, 1), main = "")
nsim <- 1000
pi0.vec <- c(0.9)
```
...
```{r sim_z_2, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
# cat("sigma.beta =", sigma.beta, "; GD =", L, "; Constraints on w:", w.lambda != 0)
sigma.beta <- 4
qvalue.pi0.list <- qvalue.list <- list()
z.pi0.list <- z.list <- list()
...
```{r gtex_z, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
# cat("sigma.beta =", sigma.beta, "; GD =", L, "; Constraints on w:", w.lambda != 0)
sigma.beta <- 3
qvalue.pi0.list <- qvalue.list <- list()
z.pi0.list <- z.list <- list()
...
for (i in 1 : nsim) {
  ...
  z <- summary$z
  sebetahat <- summary$sebetahat
  beta <- sample(c(rep(0, round(pi0 * ngene)), rnorm(round((1 - pi0) * ngene), 0, sigma.beta * sqrt(mean(sebetahat^2)))))
  betahat <- beta + sebetahat * z
...
```{r gtex_z_mix, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
# cat("sigma.beta =", sigma.beta, "; GD =", L, "; Constraints on w:", w.lambda != 0)
qvalue.pi0.list <- qvalue.list <- list()
z.pi0.list <- z.list <- list()
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    rep(0, round(pi0 * ngene)),
    rnorm(round(0.05 * ngene), 0, 1 * sqrt(mean(sebetahat^2))),
    rnorm(round(0.05 * ngene), 0, 3 * sqrt(mean(sebetahat^2)))
    ))
  betahat <- beta + sebetahat * z
...
```{r gtex_z_homo, echo = FALSE, cache = TRUE, warning = FALSE, message = FALSE}
# cat("sigma.beta =", sigma.beta, "; GD =", L, "; Constraints on w:", w.lambda != 0)
sigma.beta <- 3
qvalue.pi0.list <- qvalue.list <- list()
z.pi0.list <- z.list <- list()
