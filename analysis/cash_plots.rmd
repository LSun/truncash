...
# Noise <- cut(sd.z, breaks = c(0, z.under, z.over, Inf), labels = c("Deflated Noise", "In-between", "Inflated Noise"))
  ...
  grid.arrange(FDR.calib.plot, pi0.plot,
               top = textGrob(bquote(paste(pi[0] == .(pi0.vec[j]), "\n")), gp = gpar(fontsize = 15, font = 3))
               )
  grid.arrange(FDR.plot, power.plot,
               top = textGrob(bquote(paste("At Nominal FDR = ", .(q), "\n")), gp = gpar(fontsize = 15, font = 3))
               )
  g <- arrangeGrob(FDR.calib.plot, pi0.plot,
          arrangeGrob(FDR.plot, power.plot,
            nrow = 2, ncol = 1,
            top = textGrob(bquote(paste("At Nominal FDR = ", .(q), "\n")), gp = gpar(fontsize = 15, font = 3))
          ), ncol = 1, heights = c(1, 1, 2),
          top = textGrob(bquote(paste(pi[0] == .(pi0.vec[j]), "\n")), gp = gpar(fontsize = 15, font = 3))
      )
  ggsave(paste0("../output/fig/pi0_", pi0.vec[j], ".eps"), g, height = 10, width = 8)
}
```
