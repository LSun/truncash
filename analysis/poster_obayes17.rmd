...
for (i in 1 : 2) {
  gd.std = (-1)^i * hermite[[i]](xplot) * gd0.std
  matrix_lik_w = cbind(matrix_lik_w, gd.std)
}
plot(xplot, xplot, type = "n", xlim = c(-5, 5), ylim = range(matrix_lik_w[, 1 : 3]), bty = "n", xlab = "", ylab = "", main = "The First 2 Gaussian Derivatives")
col = c(2, 3, 4)
for (i in 1 : 3) {
  lines(xplot, matrix_lik_w[, i], col = i + 1)
}
legend("topright", col = 2 : 4, lty = 1, c(expression(
    varphi,
    paste(varphi, "'"),
    paste(varphi, '"')
  )
