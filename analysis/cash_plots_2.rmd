...
FDP.q.plot <- ggplot(data = FDP.q.ggdata, aes(x = pi0, y = FDP)) +
  ...
  labs(x = expression(pi[0]), y = bquote(paste("FDP at Nominal FDR = ", .(q)))) +
  theme(axis.title.x = element_text(size = 15),
        axis.text.x = element_text(size = 10, angle = 45, hjust = 1),
        axis.title.y = element_text(size = 15),
        axis.text.y = element_text(size = 10),
...
TDP.q.plot <- ggplot(data = TDP.q.ggdata, aes(x = pi0, y = TDP)) +
  ...
  facet_wrap(~Noise, nrow = 1, ncol = 4) +
  geom_hline(yintercept = q, col = "black", linetype = "dashed", size = 1) +
  labs(x = expression(pi[0]), y = bquote(paste("TDP at Nominal FDR = ", .(q)))) +
  theme(axis.title.x = element_text(size = 15),
        axis.text.x = element_text(size = 10, angle = 45, hjust = 1),
        axis.title.y = element_text(size = 15),
        axis.text.y = element_text(size = 10),
...
gridExtra::grid.arrange(pi0hat.plot, FDR.calib.plot, FSR.calib.plot, FDP.q.plot, TDP.q.plot, ncol = 1)
```

## Big normal: $g_2 = N\left(0, 5^2\right)$
...
## Near normal: $g_3 = 0.6 N\left(0, 1^2\right) + 0.4 N\left(0, 3^2\right)$
...
## Spikey: $g_4 = 0.4 N\left(0, 0.5^2\right) + 0.2 N\left(0, 1^2\right) + 0.2 N\left(0, 2^2\right) + 0.2 N\left(0, 3^2\right)$
...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    ...
    rnorm(round((1 - pi0) * 0.2 * ngene)),
    rnorm(round((1 - pi0) * 0.2 * ngene), 0, 2),
    rnorm(round((1 - pi0) * 0.2 * ngene), 0, 3)
    ))
  betahat <- beta + sebetahat * z
...
## Skew: $g_5 = 1/4 N\left(-2, 2^2\right) + 1/4 N\left(-1, 2^2\right) + 1/4 N\left(0, 1^2\right) + 1 / 4 N\left(1, 1^2\right)$
...
for (i in 1 : nsim) {
  ...
  sebetahat <- sebetahat / se
  pi0 <- sample(pi0.vec, 1)
  beta <- as.vector(sapply(seq(-1.5, 1.5, by = 0.5), function (x) {rnorm(round((1 - pi0) * ngene / length(seq(-1.5, 1.5, by = 0.5))), x, 0.5)}))
...
## Bimodal: $g_7 = 0.5 N\left(-1.5, 1\right) + 0.5 N\left(1.5, 1\right)$
