...
FDP.mean <- lapply(FDP.list, function (FDP.mat) {
  rbind(
    All = colMeans(FDP.mat, na.rm = TRUE),
    apply(FDP.mat, 2, tapply, Noise, mean, na.rm = TRUE)
  )
})
FDP.mean.ggdata <- melt(FDP.mean, value.name = "FDP.mean")
FDP.q975 <- lapply(FDP.list, function (FDP.mat) {
  rbind(
    All = apply(FDP.mat, 2, quantile, probs = 0.975, na.rm = TRUE),
    apply(FDP.mat, 2, tapply, Noise, quantile, probs = 0.975, na.rm = TRUE)
  )
})
FDP.q975.ggdata <- melt(FDP.q975, value.name = "FDP.q975")
FDP.q025 <- lapply(FDP.list, function (FDP.mat) {
  rbind(
    All = apply(FDP.mat, 2, quantile, probs = 0.025, na.rm = TRUE),
    apply(FDP.mat, 2, tapply, Noise, quantile, probs = 0.025, na.rm = TRUE)
  )
})
FDP.q025.ggdata <- melt(FDP.q025, value.name = "FDP.q025")
FDP.calib.ggdata <- cbind.data.frame(FDP.mean.ggdata,
  FDP.q975 = FDP.q975.ggdata$FDP.q975,
  FDP.q025 = FDP.q025.ggdata$FDP.q025
)
FDP.calib.ggdata$L1 <- as.numeric(FDP.calib.ggdata)

FDR.calib.plot <- ggplot(data = FDP.calib.ggdata, aes(x = L1, y = FDP.mean, group = Var2, col = Var2)) +
  geom_line() +
  geom_ribbon(aes(ymin = FDP.q025, ymax = FDP.q975, fill = Var2), alpha = 0.35, linetype = "blank") +
