...
```{r, cache = TRUE}
quantile.vec1 <- c(exp(seq(-30, -5, by = 0.01)), seq(0.007, 0.993, by = 0.001))
emp.cdf.Z1 <- sapply(quantile.vec, function(x) {sapply(Z.gtex, function(y) mean(y <= qnorm(x)))})
quantile.vec2 <- c(exp(seq(-5, -30, by = 0.01)))
emp.cdf.Z2 <- sapply(quantile.vec, function(x) {sapply(Z.gtex, function(y) mean(y <= -qnorm(x)))})
quantile.vec <- c(quantile.vec1, quantile.vec2)
emp.cdf.Z <- cbind(emp.cdf.Z1, emp.cdf.Z2)
```
