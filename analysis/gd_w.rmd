...
```{r}
G <- function (t) {
  0.6 * pnorm(t, 0, 0.25) + 0.3 * pnorm(t, 0, 1) + 0.1 * pnorm(t, 0, 3)
...
s <- sqrt(rchisq(1e4, 5) / 5)

method.name <- c("Kiefer-Wolfowitz", "Kernal Deconv", "Efron", "ASH", "CASH")
...
for (i in 3 : 4) {
  ...
  ## ASH
  fit.ash <- ashr::ash(X, s, method = "shrink", mixcompdist = "normal")
  ...
  ## CASH
  fit.cash <- gdash(X, s, method = "shrink")
  ...
  fit.kw <- REBayes::GLmix(Z)
  kw.plot <- Wasser(G, fit.kw, interp = "biweight")
  ...
  lines(x.plot, cash.plot, col = "red")
  lines(x.plot, ash.plot, col = "blue")
  lines(fit.bd$x, bd.plot$H, col = "green")
  lines(fit.kw$x, kw.plot$H, col = "maroon")
  lines(fit.sc$x, sc.plot$H, col = "orange")
}

ggplot(data = deconv.inf.ggdata, aes(x = x, y = cdfhat, col = method)) + geom_line() + stat_function(fun = G, col = "red") + xlim(-5, 5)
```
