...
method.name <- c("KW", "Efron", "ASH", "CASH")
method.col <- c(scales::hue_pal()(4)[1 : 2], scales::hue_pal()(5)[4 : 5])
...
for (i in 3 : 4) {
  ...
  ## CASH
  fit.cash <- gdash(X, s, gd.priority = TRUE)
...
ggplot(data = deconv.ggdata, aes(x = x, y = cdfhat, col = method, linetype = method)) + 
  ...
  labs(x = expression(theta), y = "Estimated CDF of g") +
