...
sum(p.norm.1.6 <= 0.005)
y.max <- -log(min(p.norm.1, p, p.norm.1.6))
y.max <- 20
plot(-log(p.norm.1), ylim = c(0, y.max), ylab = "-log(p)", main = expression(paste("(d): Independent ", N(0, 1))), col = "blue", cex.main = 1.5, cex.lab = 1.5)
...
plot(sample(-log(p)), ylim = c(0, y.max), ylab = "-log(p)", main = expression("(e): Hist. (c)"), cex.main = 1.5, cex.lab = 1.5)
abline(h = -log(0.005), col = "red", lwd = 2)
abline(h = -log(pnorm(-sqrt(2 * log(1e4))) * 2), col = "orange", lwd = 2)
abline(h = -log(0.05 / 1e4), col = "yellow", lwd = 2)
