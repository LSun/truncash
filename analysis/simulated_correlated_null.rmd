...
w_prior = function(lambda, rho, order) {
  w_prior = rep(lambda, order)
  w_prior[seq(2, order, by = 2)] = lambda / rho^{1 : (order / 2)}
  return(w_prior)
}
...
# w.std = w.mosek(matrix_lik_w = gd.std.mat(z, order), w_prior = w_prior(lambda, rho, order))
...
legend("topleft", inset = c(0, 0.075), lty = 1, col = "red", "N (0, 1)", bty = "n")
