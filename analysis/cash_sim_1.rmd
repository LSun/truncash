---
title: "CASH Simulation, Part 1: Overall"
author: "Lei Sun"
date: 2017-11-09
...
We are doing large-scale simulation using `CASH` and other methods, with emphasis on the accuracy of FDR calibration.

## All data sets on average, including both over-dispersion, under-dispersion, and else
...
for (i in 1 : nrow(z.mat)) {
  n = length(z.mat[i, ])
  sigma_n = sqrt(mean(se.mat[i, ]^2))
  sigma_s.1 = sigma_n
  sigma_s.2 = 2 * sigma_n
  g = ashr::normalmix(pi = c(0.6, 0.3, 0.1), mean = 0, sd = c(0, sigma_s.1, sigma_s.2))
  beta = sample.g(g, n)$beta
  response = (beta != 0)
  betahat = beta + se.mat[i, ] * z.mat[i, ]
  pvalue = (1 - pnorm(abs(betahat / se.mat[i, ]))) * 2
  fit.ash = ashr::ash(betahat, se.mat[i, ], mixcompdist = "normal", method = "fdr")
  fit.gdash = gdash(betahat, se.mat[i, ])
...
plot(pFDR.nominal, pFDR.nominal, ylim = c(0, 2 * max(pFDR.nominal)), type = "n", xlab = "Nominal Positive False Discovery Rate", ylab = "Average Positive False Discovery Proportion", main = "Calibration of Positive FDR (Overall)")
