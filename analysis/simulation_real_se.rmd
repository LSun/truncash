...
```{r null fdp, cache = TRUE, echo = FALSE}
fdp = function (FDR, predictor) {
  return(any(predictor <= FDR))
}
...
for (i in 1 : m) {
  ...
  fdp.ash[[i]] = sapply(fdr.list, fdp, predictor = qvalue.ash)
  fdp.gdash[[i]] = sapply(fdr.list, fdp, predictor = qvalue.gdash)
  fdp.qvalue[[i]] = sapply(fdr.list, fdp, predictor = qvalue.qvalue)
  fdp.BH[[i]] = sapply(fdr.list, fdp, predictor = BH[[i]])
}
```

```{r null average fdp summary, cache = TRUE, echo = FALSE}
...
```{r null average fdp plotting, echo = FALSE}
par(mfrow = c(1, 2))

plot(fdr.list, fdr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Discovery Proportion", main = "Calibration of FDR")
...
plot(fdr.list, fdr.list, type = "n",
     xlim = c(0, 0.2), ylim = c(0, 0.2), xlab = "Nominal False Discovery Rate", ylab = "Average False Discovery Proportion", main = "Calibration of FDR: Zoom-in")
...
```{r null fsp, cache = TRUE, echo = FALSE}
fsp = function (FSR, observation, predictor) {
  fsp = 0
  if (any(predictor <= FSR)) {
    fsp = mean(observation[predictor <= FSR] != 0)
  }
  return(fsp)
}
...
for (i in 1 : m) {
  ...
  fsp.ash[[i]] = sapply(fsr.list, fsp, observation = betahat[[i]], predictor = svalue.ash)
  fsp.gdash[[i]] = sapply(fsr.list, fsp, observation = betahat[[i]], predictor = svalue.gdash)
}
```

```{r null average fsp summary, cache = TRUE, echo = FALSE}
...
```{r null average fsp plotting, echo = FALSE}
par(mfrow = c(1, 2))
...
```{r zero fsp, cache = TRUE, echo = FALSE}
...
```{r zero average fdp summary, cache = TRUE, echo = FALSE}
mean.fdp.ash = colMeans(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE))
sd.fdp.ash = apply(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fdp.gdash = colMeans(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE))
sd.fdp.gdash = apply(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r zero average fdp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fdr.list, fdr.list, type = "n",
...
```{r zero average fsp summary, cache = TRUE, echo = FALSE}
mean.fsp.ash = colMeans(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE))
sd.fsp.ash = apply(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fsp.gdash = colMeans(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE))
sd.fsp.gdash = apply(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r zero average fsp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fsr.list, fsr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Sign Proportion", main = "Calibration of S values")
...
```{r small fsp, cache = TRUE, echo = FALSE}
fsp = function (FSR, true, observation, predictor) {
  discovery = predictor <= FSR
  return(mean(sign(observation[discovery]) != true[discovery]))
}
...
```{r small average fdp summary, cache = TRUE, echo = FALSE}
mean.fdp.ash = colMeans(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE))
sd.fdp.ash = apply(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fdp.gdash = colMeans(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE))
sd.fdp.gdash = apply(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r small average fdp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fdr.list, fdr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Discovery Proportion", main = "Calibration of pFDR")
...
```{r small average fsp summary, cache = TRUE, echo = FALSE}
mean.fsp.ash = colMeans(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE))
sd.fsp.ash = apply(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fsp.gdash = colMeans(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE))
sd.fsp.gdash = apply(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r small average fsp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fsr.list, fsr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Sign Proportion", main = "Calibration of S values")
...
```{r medium fsp, cache = TRUE, echo = FALSE}
fsp = function (FSR, true, observation, predictor) {
  discovery = predictor <= FSR
  return(mean(sign(observation[discovery]) != true[discovery]))
}
...
```{r medium average fdp summary, cache = TRUE, echo = FALSE}
mean.fdp.ash = colMeans(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE))
sd.fdp.ash = apply(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fdp.gdash = colMeans(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE))
sd.fdp.gdash = apply(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r medium average fdp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fdr.list, fdr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Discovery Proportion", main = "Calibration of pFDR")
...
```{r medium average fsp summary, cache = TRUE, echo = FALSE}
mean.fsp.ash = colMeans(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE))
sd.fsp.ash = apply(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fsp.gdash = colMeans(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE))
sd.fsp.gdash = apply(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r medium average fsp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fsr.list, fsr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Sign Proportion", main = "Calibration of S values")
...
```{r big fsp, cache = TRUE, echo = FALSE}
fsp = function (FSR, true, observation, predictor) {
  discovery = predictor <= FSR
  return(mean(sign(observation[discovery]) != true[discovery]))
}
...
```{r big average fdp summary, cache = TRUE, echo = FALSE}
mean.fdp.ash = colMeans(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE))
sd.fdp.ash = apply(matrix(unlist(fdp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fdp.gdash = colMeans(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE))
sd.fdp.gdash = apply(matrix(unlist(fdp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r big average fdp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fdr.list, fdr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Discovery Proportion", main = "Calibration of pFDR")
...
```{r big average fsp summary, cache = TRUE, echo = FALSE}
mean.fsp.ash = colMeans(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE))
sd.fsp.ash = apply(matrix(unlist(fsp.ash), nrow = m, byrow = TRUE), 2, sd)
mean.fsp.gdash = colMeans(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE))
sd.fsp.gdash = apply(matrix(unlist(fsp.gdash), nrow = m, byrow = TRUE), 2, sd)
...
```{r big average fsp plotting, echo = FALSE}
par(mfrow = c(1, 2))
plot(fsr.list, fsr.list, type = "n",
     xlim = c(0, 1), ylim = c(0, 1), xlab = "Nominal False Discovery Rate", ylab = "Average False Sign Proportion", main = "Calibration of S values")
