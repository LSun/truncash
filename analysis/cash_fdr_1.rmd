...
where
$$
g = 0.5\delta_0 + 0.5N(0, \sigma_n^2) \ .
$$
We consider three scenarios for the noise $z_j$:
1. Correlated, simulated from real data;
2. Independent;
3. Correlated, simulated from synthetic data.
...
for (i in 1 : nsim) {
  ...
  z <- summary$z
  sd.n <- sqrt(mean(z^2))
  beta[[i]] <- sample(c(rep(0, pi0 * ngene), rnorm((1 - pi0) * ngene, 0, snr * sd.n)))
...
```{r cor FDR tail, fig.asp = 1, echo = FALSE}
cutoff <- 0.01
loc <- FDR.nominal == cutoff
FDP.cutoff <- cbind(
  BH = pFDP.mat.BH[, loc],
...
plot(FDP.cutoff[, "BH"], FDP.cutoff[, "CASH"],
     ...
     xlim = range(FDP.cutoff[, c("BH", "CASH")]),
     ylim = range(FDP.cutoff[, c("BH", "CASH")]),
     xlab = "BH",
     ylab = "CASH",
...
plot(FDP.cutoff[, "ASH"], FDP.cutoff[, "CASH"],
     col = rgb(sd.z >= 1.1, 0, sd.z <= 0.9, (sd.z - min(sd.z)) / (max(sd.z) - min(sd.z)) * 0.8 + 0.2),
     xlim = range(FDP.cutoff[, c("ASH", "CASH")]),
     ylim = range(FDP.cutoff[, c("ASH", "CASH")]),
     xlab = "ASH",
     ylab = "CASH",
     main = bquote(paste("False Discovery Proportions at ", q == .(cutoff), " Cutoff")),
     pch = 19,
