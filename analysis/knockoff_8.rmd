...
for (i in 1 : m) {
  ...
  W_f_e <- pmax(Z[orig], Z[orig + p]) * sign(Z[orig] - Z[orig + p])

  random.order.variable <- sample(2 * p)
  Z <- knockoff:::lasso_max_lambda(cbind(X, Xk_f_s)[, random.order.variable], y, method = "glmnet", family = "gaussian")
  Z <- Z[order(random.order.variable)]
  W_f_s <- pmax(Z[orig], Z[orig + p]) * sign(Z[orig] - Z[orig + p])

  ## Knockoff
  W[[i]] <- cbind(
    equi_marginal <- abs(t(X) %*% y) - abs(t(Xk_f_e) %*% y),
    sdp_marginal <- abs(t(X) %*% y) - abs(t(Xk_f_s) %*% y),
    equi_lasso <- knockoff::stat.glmnet_lambdasmax(X, Xk_f_e, y),
    equi_lasso_random <- W_f_e,
    sdp_lasso <- knockoff::stat.glmnet_lambdasmax(X, Xk_f_s, y),
    sdp_lasso_random <- W_f_s
  )
