...
for (i in seq(method.name)) {
  ...
  arrows(x0 = seq(2), x1 = seq(2), y0 = c(mean(result.fewsmall[, i + 6]), mean(result.manysmall[, i + 6])) - 2 * c(sd(result.fewsmall[, i + 6]), sd(result.manysmall[, i + 6])) / sqrt(m), y1 = c(mean(result.fewsmall[, i + 6]), mean(result.manysmall[, i + 6])) + 2 * c(sd(result.fewsmall[, i + 6]), sd(result.manysmall[, i + 6])) / sqrt(m), length = 0.05, angle = 90, code = 3, col = method.col[i])
}
legend("top", horiz = TRUE, method.name, col = method.col, pch = seq(method.name), lty = 1)
```
