...
for (i in 1 : nsim) {
  ...
  fit.gdash = gdash(betahat, sebetahat, gd.ord = 10)
...
z.over <- 1.05
z.under <- 0.95
method.col <- wesanderson::wes_palette(name = "Zissou1", length(method.name))
```
...
for (j in seq(length(pi0.vec))) {
  ...
  FDR.calib.plot <- ggplot(data = FDP.summary.pi0.method, aes(x = FDR, y = FDP, group = Method, col = Method)) +
  geom_line() +
  ...
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5, size = 15), axis.title.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.x = element_text(size = 15), axis.text.y = element_text(size = 15), strip.text = element_text(size = 15))
