...
for (i in 1 : nsim) {
  ...
  beta <- sample(c(
    ...
    rnorm(round((1 - pi0) * ngene), 0, 3 * sqrt(mean(sebetahat^2)))
    ))
  betahat <- beta + sebetahat * z
  
  pvalue = pnorm(-abs(betahat / sebetahat)) * 2

  ## different methods
  fit.ash = ashr::ash(betahat, sebetahat, mixcompdist = "normal", method = "fdr")
  fit.gdash = gdash(betahat, sebetahat, gd.ord = 10, w.rho = 0.5)
...
for (j in seq(length(pi0.vec))) {
  ...
  colnames(FDP.summary.pi0.method.noise) <- c(
    "Noise", "Method", "FDR", "FDP", "sd", "n", "q975", "q025"
  )
  ...
  geom_line() +
  geom_ribbon(aes(ymin = FDP - sd, ymax = FDP + sd, fill = Method), alpha = 0.2, linetype = "blank") +
  ...
  print(pi0.plot)
  ggsave("../output/fig/pi0_sep.eps", pi0.plot, width = 7.5, height = 3)
  print(FDR.calib.plot)
  ggsave("../output/fig/fdr_calib.pdf", FDR.calib.plot, width = 7.5, height = 3)
  grid.arrange(FDR.plot, power.plot, top = textGrob(bquote(paste("At Nominal FDR = ", .(q))),
                                                    gp = gpar(fontsize = 15, font = 3)))
  g <- arrangeGrob(FDR.plot, power.plot, top = textGrob(bquote(paste("At Nominal FDR = ", .(q))), gp = gpar(fontsize = 15, font = 3)))
