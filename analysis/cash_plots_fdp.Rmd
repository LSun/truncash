...
# Noise <- cut(sd.z, breaks = c(0, quantile(sd.z, probs = 1 : 2 / 3), Inf), labels = c("Deflated Noise", "In-between", "Inflated Noise"))
  ...
  grid.arrange(FDR.plot, power.plot, top = textGrob(bquote(paste("At Nominal FDR = ", .(q))), gp = gpar(fontsize = 15, font = 3)))
  ...
  g <- arrangeGrob(FDR.plot, power.plot, top = textGrob(bquote(paste("At Nominal FDR = ", .(q)), "\n"), gp = gpar(fontsize = 15, font = 3)))
  ggsave("../output/fig/fdp_tpp_sep.eps", g, width = 7.5, height = 6)
  g <- arrangeGrob(FDR.calib.plot, g, heights = 1 : 2.2)
}
```
